...
| Точка | Когда | Что обновить | Файл-источник |

|-------|-------|--------------|---------------|

| 1 | После инициализации | Данные хостела (адрес, телефон) | `src/constants/hotel.ts` |

| 2 | После компонентов | Тексты в карточках номеров | Компоненты в `src/components/` |

| 3 | После страниц | Все тексты на страницах | Страницы в `src/app/` |

| 4 | Перед интеграциями | API ключи | `.env` |

| 5 | Перед деплоем | Реальные цены и описания | `prisma/seed.ts`, константы |---
...
| [TECH_STACK.md](TECH_STACK.md) | Структура проекта, API, БД, деплой |

| [DESIGN_GUIDELINES.md](DESIGN_GUIDELINES.md) | UI компоненты, цвета, шрифты, wireframes |